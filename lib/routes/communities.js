...
router
  .get('/advance', (req, res, next) => { 
    console.log('inside advance router');
    User
      .findById(req.user.id)
      .then(user => {
        console.log('inside /advance route, user.communityId:', user.communityId);
      ...
      .then(id => {
        Experience
          ...
          .sort([['_id', -1]])
          .then(experiences => {
            console.log('these are sorted experiences: ', experiences);
